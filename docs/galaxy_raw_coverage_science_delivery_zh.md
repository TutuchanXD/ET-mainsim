...
writer 先在写入位置生成 partial，完成 HDF5 readback/contract 验证后才原子 publish 到 final。
一个 shard 的 raw 和四个 coadd 成员必须完整；最终共享交付根中的 `.partial`、`.lock`、
`.incoming`、未知 HDF5 或不完整集合均不可交付，且正式生产不从 partial 续跑。node-local
scratch 是否清理由 staged publisher 自身负责，不是 campaign QC 可以直接遍历审计的目录。
...
target/shard 以及自身 scratch 清理或发布前条件不满足均失败关闭；发布到共享根后，campaign
QC 再审计共享根中的 incoming/lock/partial 残留。两者的责任边界不同，但都必须通过，才可
放行交付。staged 模式改善共享盘写入鲁棒性，不会改变 `final_dn` 的物理定义或本节的科学
输入/输出语义。
...
软件身份以实际导入 source checkout 的 `root`、git `commit` 和 `dirty` 状态为权威。
provenance 中的 `version` 由运行环境的 `importlib.metadata` 提取，可能只是遗留的
distribution metadata；它可与 source `pyproject.toml`、prepare-time 或 runtime 的标签不同，
**不能单独用于判断代码漂移**。若标签不同，run-specific 说明必须并列记录所有实际值，
并以 commit/clean/source-root 的全量审计结论说明是否存在真实代码混用。
...
2. target 清单、duration、raw/coadd cadence、stamp shape、pixel scale、正式软件 source
   root、commit、clean/dirty provenance，以及所有实际 distribution-version 标签与其差异
   的验收说明；
