...
这不是完整科学链路验收。当前尚未关闭的门包括：PSF normalization、crop 与
`captured_flux_fraction` deterministic golden，standalone selection sidecar，
shared-exposure crop，跨 backend/ensemble tolerance，以及最终 RMS/CDPP 产品比较。
因此不得把“Stage 2 selection identity 已通过”简写成“full-frame、stamp 与
legacy 已完整科学对齐”。
...
- 把完整 PSF/JI selection truth 作为独立、可长期 readback 的 sidecar 持久化；
- 完成相同 physical source 的 PSF support、crop、normalization 和
  `captured_flux_fraction` golden；
- 完成 shared-exposure stamp、absolute-detector RNG 与 pixel-first component 产品；
- 在查看结果前预注册 deterministic、statistical、跨 backend 与最终科学指标容差。
